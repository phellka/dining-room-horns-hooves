...
namespace DiningRoomDatabaseImplement.Implements
{
    public class CutleryStorage : ICutleryStorage
    {
        ...
        public List<CutleryViewModel> GetFilteredList(CutleryBindingModel model)
        {
            ...
            using var context = new DiningRoomDatabase();
            return context.Cutleries.Where(rec => rec.OrderId == model.CulteryOrder).Select(CreateModel).ToList();
        }
        public CutleryViewModel GetElement(CutleryBindingModel model)
        {
            ...
            using var context = new DiningRoomDatabase();
            var cutlery = context.Cutleries.FirstOrDefault(rec => rec.Id == model.Id);
        ...
        public void Update(CutleryBindingModel model)
        {
            ...
            try
            {
                var element = context.Cutleries.FirstOrDefault(rec => rec.Id == model.Id);
                if (element == null)
                {
        ...
        {
            using var context = new DiningRoomDatabase();
            Cutlery element = context.Cutleries.FirstOrDefault(rec => rec.Id == model.Id);
            if (element != null)
            {
